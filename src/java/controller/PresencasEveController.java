...
public class PresencasEveController implements Serializable {
    ...
    public PresencasEve getSelected() {
        if (current == null) {
            current = new PresencasEve();
            current.setPresencasEvePK(new model.PresencasEvePK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String prepareView() {
        current = (PresencasEve) getItems().getRowData();
    ...
    public String create() {
        try {
            current.getPresencasEvePK().setIdevento(current.getEventos().getIdevento());
            current.getPresencasEvePK().setIdparticipante(current.getParticipantes().getIdparticipante());
            getFacade().create(current);
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("PresencasEveCreated"));
    ...
    public String update() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("PresencasEveUpdated"));
            return "View";
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
            return null;
    ...
    private void performDestroy() {
        try {
            ...
            JsfUtil.addSuccessMessage(ResourceBundle.getBundle("/Bundle").getString("PresencasEveDeleted"));
        } catch (Exception e) {
            JsfUtil.addErrorMessage(e, ResourceBundle.getBundle("/Bundle").getString("PersistenceErrorOccured"));
        }
    }
    ...
    public static class PresencasEveControllerConverter implements Converter {
        ...
        public String getAsString(FacesContext facesContext, UIComponent component, Object object) {
            ...
            if (object instanceof PresencasEve) {
                ...
                throw new IllegalArgumentException("object " + object + " is of type " + object.getClass().getName() + "; expected type: " + PresencasEve.class.getName());
            }
        }
